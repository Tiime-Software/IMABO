...
from hpobench.util.data_manager import (
    BostonHousingData,
    ProteinStructureData,
    YearPredictionMSDData,
)
...
class BayesianNeuralNetworkBenchmark(AbstractBenchmark):
    ...
    def objective_function(
        ...
        **kwargs,
        ...
        valid_loss_sample = self._neg_log_likelihood_sample(
            self.valid_targets, mean_pred, var_pred
        )
        ...
        return {
            ...
            "function_value_sample": valid_loss_sample,
            "cost": cost,
    ...
    def objective_function_test(
        ...
        **kwargs,
        ...
        test_loss_sample = self._neg_log_likelihood_sample(
            self.test_targets, mean_pred, var_pred
        )

        cost = time.time() - start
        return {
            "function_value": float(test_loss),
            "function_value_sample": test_loss_sample,
            "cost": cost,
            "info": {"fidelity": fidelity},
        }
    ...
    def _neg_log_likelihood(
        targets: np.ndarray, mean_pred: np.ndarray, var_pred: np.ndarray
    ) -> np.ndarray:
        """Compute the negative log likelihood for normal distributions."""
        var_pred = np.clip(var_pred, a_min=1e-10, a_max=np.inf)
        nll = [
            stats.norm.logpdf(targets[i], loc=mean_pred[i], scale=np.sqrt(var_pred[i]))
            for i in range(targets.shape[0])
        ]
        return -np.mean(nll)

    @staticmethod
    def _neg_log_likelihood_sample(
        targets: np.ndarray, mean_pred: np.ndarray, var_pred: np.ndarray
    ) -> list:
        """Compute the negative log likelihood for normal distributions."""
        var_pred = np.clip(var_pred, a_min=1e-10, a_max=np.inf)
    ...
    def get_configuration_space(seed: Union[int, None] = None) -> CS.ConfigurationSpace:
        ...
        cs.add_hyperparameters(
            [
                ...
                CS.UniformFloatHyperparameter(
                    "burn_in", lower=0.0, upper=0.8, default_value=0.3
                ),
    ...
    def get_fidelity_space(seed: Union[int, None] = None) -> CS.ConfigurationSpace:
        ...
        fidel_space.add_hyperparameters(
            [
                CS.UniformIntegerHyperparameter(
                    "budget", lower=500, upper=10000, default_value=10000, log=False
                )
            ]
        )
...
class BNNOnToyFunction(BayesianNeuralNetworkBenchmark):
    ...
    def get_data(self):
        ...
        def toy_function(x):
            ...
            y = (
                x
                + 0.3 * np.sin(2 * np.pi * (x + eps))
                + 0.3 * np.sin(4 * np.pi * (x + eps))
                + eps
            )
            return y
