...
from custom_score_functions.accuracy_sample import sample_accuracy_score
...
metrics = dict(
    ...
    sample_acc=sample_accuracy_score,
)
...
class MLBenchmark(AbstractBenchmark):
    ...
    def _train_objective(
        ...
        train_idx = self.rng.choice(
            np.arange(len(train_X)), size=int(subsample * len(train_X))
        )
    ...
    def objective_function(
        ...
        **kwargs
        ...
        model, model_fit_time, train_loss, train_scores, train_score_cost = (
        ...
        val_sample_acc = val_scores["sample_acc"]
    ...
    def objective_function_test(
        ...
        **kwargs
        ...
        for k, v in self.scorers.items():
            ...
            test_score_cost[k] = time.time() - _start
        test_loss = 1 - test_scores["acc"]
        test_sample_acc = test_scores["sample_acc"]
        test_acc = test_scores["acc"]
