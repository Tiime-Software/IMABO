...
try:
    ...
    print(
        "oslo_concurrency not installed, can't download datasets for nasbench201 (not needed for containers)"
    )
...
class DataManager(abc.ABC, metaclass=abc.ABCMeta):
    ...
    @lockutils.synchronized(
        ...
        external=True,
        lock_path=f"{hpobench.config_file.cache_dir}/lock_download_file",
        delay=0.5,
    )
    ...
    @lockutils.synchronized(
        "not_thread_process_safe",
        external=True,
        lock_path=f"{hpobench.config_file.cache_dir}/lock_unzip_file",
        delay=0.5,
    )
...
class MNISTData(HoldoutDataManager):
    ...
    def __load_data(self, filename: str, images: bool = False) -> np.ndarray:
        ...
        if not save_fl.exists():
            self.logger.debug(
                f"Downloading {self._url_source + filename} " f"to {save_fl}"
            )
...
class MNISTDataCrossvalidation(MNISTData, CrossvalidationDataManager):
    ...
    def load(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        X_train, y_train, X_val, y_val, X_test, y_test = super(
            MNISTDataCrossvalidation, self
        ).load()
...
class SVHNData(DataManager):
    ...
    def __load_data(
        self, filename_train: str, filename_test: str
        ...
        def __load_x_y(file_name):
            ...
            if not save_fl.exists():
                self.logger.debug(
                    f"Downloading {self._url_source + file_name}" f" to {save_fl}"
                )
...
class NASBench_201Data(DataManager):
    ...
    def __init__(self, dataset: str):
        ...
        self.files = [
            f"NAS-Bench-201-v1_1-096897_{dataset}.json" for dataset in all_datasets
        ]
    ...
    @lockutils.synchronized(
        "not_thread_process_safe",
        external=True,
        lock_path=f"{hpobench.config_file.cache_dir}/lock_nasbench_201_data",
        delay=0.5,
    )
    def _download(self):
        ...
        file_is_missing = not all(
            [(self._save_dir / file).exists() for file in self.files]
        )
        ...
        if not file_is_missing:
            ...
            self.logger.info(
                f"NasBench201DataManager: Start downloading data from {self._url_source} "
                f"to {self._save_dir}"
            )
    ...
    def load(self) -> Dict:
        ...
        self.logger.info(
            f"NasBench201DataManager: Data successfully loaded after {time() - t:.2f}"
        )
...
class NASBench_101DataManager(DataManager):
    def __init__(self, data_path: Union[str, Path, None] = None):
        ...
        self.save_dir = (
            (hpobench.config_file.data_dir / "nasbench_101")
    ...
    @lockutils.synchronized(
        "not_thread_process_safe",
        external=True,
        lock_path=f"{hpobench.config_file.cache_dir}/lock_nasbench_101_data",
        delay=0.5,
    )
    ...
    def download(self) -> None:
        ...
        if not (self.save_dir / self.fname).exists():
            self.logger.info(
                f"NasBench101DataManager: File {self.save_dir / self.fname} not found."
                f" Start downloading."
            )
            self._download(save_to=self.save_dir / self.fname)
        else:
            self.logger.info(
                "NasBench101DataManager: Data already available. Skip downloading."
            )
...
class SurrogateDataManger(DataManager):
    def __init__(self, dataset: str):
        ...
        assert dataset in allowed_datasets, (
            f"Requested data set is not supported. Must be one of "
        ...
        self.url_source = (
            "https://www.automl.org/wp-content/uploads/2019/05/surrogates.tar.gz"
        )
    ...
    @lockutils.synchronized(
        "not_thread_process_safe",
        external=True,
        lock_path=f"{hpobench.config_file.cache_dir}/lock_surrogates_data",
        delay=0.5,
    )
    ...
    @lockutils.synchronized(
        "not_thread_process_safe",
        external=True,
        lock_path=f"{hpobench.config_file.cache_dir}/lock_surrogates_unzip_data",
        delay=0.5,
    )
    ...
    def load(self):
        self.logger.info(
            f"Start to load the data from {self.save_dir} for dataset {self.dataset}"
        )
        ...
        if not (self.obj_fn_file.exists() or self.cost_file.exists()):
            self.logger.info(
                f"One of the files {self.obj_fn_file} and {self.cost_file} not found."
            )
        ...
        self.logger.info(
            f"Finished loading the data for paramenet - dataset: {self.dataset}"
        )
...
class BostonHousingData(HoldoutDataManager):
    ...
    def load(self):
        ...
        self.logger.info(
            f"BostonHousingDataManager: Data successfully loaded after {time() - t:.2f}"
        )
    ...
    @lockutils.synchronized(
        "not_thread_process_safe",
        external=True,
        lock_path=f"{hpobench.config_file.cache_dir}/lock_protein_structure_data",
        delay=0.5,
    )
    def _download(self):
        """
        ...
        if (self._save_dir / "housing.data").exists():
            ...
            self.logger.info(
                f"BostonHousingDataManager: Start downloading data from {self.url_source} "
                f"to {self._save_dir}"
            )
...
class ProteinStructureData(HoldoutDataManager):
    ...
    def load(self):
        ...
        self.logger.info(
            f"ProteinStructureDataManager: Data successfully loaded after {time() - t:.2f}"
        )
    ...
    def _download(self):
        ...
        if (self._save_dir / "CASP.csv").exists():
            ...
            self.logger.info(
                f"ProteinStructureDataManager: Start downloading data from {self.url_source} "
                f"to {self._save_dir}"
            )
...
class YearPredictionMSDData(HoldoutDataManager):
    ...
    def load(
        ...
        X_trn, y_trn, X_val, y_val, X_tst, y_tst = self._load()
        self.logger.info(
            f"YearPredictionMSDDataManager: Data successfully loaded after {time() - t:.2f}"
        )
    ...
    @lockutils.synchronized(
        "not_thread_process_safe",
        external=True,
        lock_path=f"{hpobench.config_file.cache_dir}/lock_year_prediction_data",
        delay=0.5,
    )
    def _download(self):
        # Check if data is already downloaded.
        ...
        if (self._save_dir / "YearPredictionMSD.txt").exists():
            ...
            self.logger.info(
                f"YearPredictionMSDDataManager: Start downloading data from {self.url_source} "
                f"to {self._save_dir}"
            )
...
class TabularDataManager(DataManager):
    def __init__(
        self, model: str, task_id: [int, str], data_dir: [str, Path, None] = None
    ):
        ...
        self.parquet_file = (
            self._save_dir
            / self.task_id
            / f"{self.model}_{self.task_id}_data.parquet.gzip"
        )
        self.metadata_file = (
            self._save_dir / self.task_id / f"{self.model}_{self.task_id}_metadata.json"
        )
    ...
    def load(self):
        ...
        self._download_file_with_progressbar(
            self.url_to_use, self._save_dir / f"{self.model}.zip"
        )
